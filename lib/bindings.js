...
(function () {
	...
	ENetPeer.prototype.createWriteStream = function (channel) {
		...
		s._write = function (buf, enc, next) {
			...
			peer.send(channel, packet, function (err) {
				/* returning the error will raise error event on stream,
				   application should listen for the event otherwise it will be an uncaught exception
				*/
				if (err) {
					connected = false;
					next("peer-not-connected");
	...
	ENetPeer.prototype.createReadStream = function (channel) {
		...
		peer.once("disconnect", function (data) {
