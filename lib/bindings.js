...
(function () {
	...
	function ENetPacket() {
		...
		if (arguments.length > 0 && typeof arguments[0] == 'string') {
			return new ENetPacket(new Buffer(arguments[0]), arguments[1]);
		}
	}
	...
	ENetPacket.prototype._attachFreeCallback = function (free_ptr) {
		jsapi_.packet_set_free_callback(this._pointer, free_ptr);
	};

	ENetPacket.prototype.flags = function () {
		if (!this._pointer) return 0;
		return jsapi_.packet_flags(this._pointer);
	};

	ENetPacket.prototype.wasSent = function () {
	...
	function ENetPeer(pointer) {
		var peer = this;
		if (pointer) peer._pointer = pointer;
		else throw ("ENetPeer null pointer");
		events.EventEmitter.call(peer);
		peer.setMaxListeners(0);

		peer._packet_free_func_ptr = ENETModule["Runtime_addFunction"](function (packet_ptr) {
			//grab the callback from peer._queued_packets, call its callback indicate if sent flag
			//delete from peer._queued_packets
			var packet, callback;
			Object.keys(peer._packet_callback_functions).forEach(function (ptr) {
				//keys are strings
				if (Number(ptr) === packet_ptr) {
					packet = new ENetPacket(packet_ptr);
					callback = peer._packet_callback_functions[ptr];
					delete peer._packet_callback_functions[ptr];
		...
		peer._packet_callback_functions = {}; //packet pointers and their free callback function
	}
	...
	ENetPeer.prototype.send = function (channel, packet, callback) {
		...
		//todo - check packet._pointer !== 0

		if (typeof callback === 'function') {
			packet._attachFreeCallback(peer._packet_free_func_ptr);
			peer._packet_callback_functions[packet._pointer] = callback;
		}
	...
	ENetPeer.prototype._delete = function (emitDisconnect, disconnectData) {
		var peer = this;
		if (!peer._pointer) return;
		if (peer._host) delete peer._host.connectedPeers[peer._pointer];
		peer._pointer = 0;
		if (peer._packet_free_func_ptr) ENETModule["Runtime_removeFunction"](peer._packet_free_func_ptr);
		delete peer._packet_callback_functions;
