...
(function () {
	...
	function ENetHost(address, maxpeers, maxchannels, bw_down, bw_up, host_type, custom_socket) {
		...
		case "custom":
			...
			stream = ENETModule["CreateStream"]({
				addrlen: 16, //ipv4 socket address size
				connected: false,
				stream: false,
				dgram: true,
			...
			pointer = jsapi_.enet_host_from_socket(stream.fd, 0, maxpeers || 128, maxchannels || 5, bw_down ||
				0,
				bw_up ||
				0);
			break;
		...
		stream = ENETModule["GetStream"](socketfd);
